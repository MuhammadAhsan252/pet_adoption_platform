class CatNameGeneratorController < ApplicationController
  layout false
  def index
  end
  ...
  def build_prompt
    ...
    size        = params[:size] == "any" ? "any size cat" : "#{params[:size]} sized cat"
    ...
    prompt = <<~PROMPT
      Generate 8 creative and unique cat names with the following criteria:
      - Style: #{category}
      - Gender: #{gender}
      - Size: #{size}
