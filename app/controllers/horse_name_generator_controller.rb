class HorseNameGeneratorController < ApplicationController
  layout false
  def index
  end
  ...
  def build_prompt
    ...
    size        = params[:size] == "any" ? "any size horse" : "#{params[:size]} sized horse"
    ...
    prompt = <<~PROMPT
      Generate 8 creative and unique horse names with the following criteria:
      - Style: #{category}
      - Gender: #{gender}
      - Size: #{size}
